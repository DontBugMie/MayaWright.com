...
const HeadAqua: React.FC<props> = ({ boundary, count }) => {
  const { nodes, materials } = useGLTF<CustomGLTF>('./loaders/head-transformed.glb');
        ...
        return (
          <object3D key={index} position={[head.position.x,5,head.position.z]}>
            <RigidBody gravityScale={1}>
              <mesh geometry={nodes.FaceBuilderHead002.geometry} material={materials.material_2}  rotation={[1.544, 0.019, 0.022]} scale={0.5} />
            </RigidBody>
          </object3D>
        )
      })}
...
useGLTF.preload('./loaders/head-transformed.glb')

export default HeadAqua;