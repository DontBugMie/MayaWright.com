...
const HeadWhite: React.FC<props> = ({ boundary, count }) => {
  const { nodes, materials } = useGLTF<CustomGLTF>('./loaders/head_white-transformed.glb');
      ...
      {heads.map((head, index) => {
        return (
          <object3D key={index} position={[head.position.x,5,head.position.z]}>
            ...
            <mesh geometry={nodes.FaceBuilderHead002.geometry} material={materials.material_1}  rotation={[1.544, 0.019, 0.022]} scale={0.5} />
            </RigidBody>
          </object3D>
        )
      })}
...
useGLTF.preload('./loaders/head_white-transformed.glb')
